...
func projectaction_Save(r dm.ProjectAction, usr string) error {
	...
	// If there are fields below, create the methods in dao\projectaction_impl.go  r.NoEstimationSessions,err = ProjectAction_NoEstimationSessions_OnStore_impl (r.NoEstimationSessions,r,usr)
...
func projectaction_Fetch(tsql string) (int, []dm.ProjectAction, dm.ProjectAction, error) {
	...
	for i := 0; i < noitems; i++ {
		...
		// If there are fields below, create the methods in adaptor\ProjectAction_impl.go   recItem.NoEstimationSessions  = ProjectAction_NoEstimationSessions_OnFetch_impl (recItem)//
		// Dynamically generated 15/02/2023 by matttownsend (Matt Townsend) on silicon.local
		// END
		///
		//Add to the list
