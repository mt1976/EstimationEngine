...
func project_Save(r dm.Project, usr string) error {
	...
	// If there are fields below, create the methods in dao\project_impl.go  r.ProjectID,err = Project_ProjectID_OnStore_impl (r.ProjectID,r,usr)  r.Description,err = Project_Description_OnStore_impl (r.Description,r,usr)  r.NoEstimationSessions,err = Project_NoEstimationSessions_OnStore_impl (r.NoEstimationSessions,r,usr)
...
func project_Fetch(tsql string) (int, []dm.Project, dm.Project, error) {
	...
	for i := 0; i < noitems; i++ {
		...
		// If there are fields below, create the methods in adaptor\Project_impl.go   recItem.ProjectID  = Project_ProjectID_OnFetch_impl (recItem)   recItem.Description  = Project_Description_OnFetch_impl (recItem)   recItem.NoEstimationSessions  = Project_NoEstimationSessions_OnFetch_impl (recItem)//
		// Dynamically generated 15/02/2023 by matttownsend (Matt Townsend) on silicon.local
		// END
		///
		//Add to the list
