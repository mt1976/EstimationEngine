...
func Project_GetFilteredLookup(requestObject string,requestField string) []dm.Lookup_Item {
	...
	usage := "Defines a filter for a lookup list of Project records, when requested by "+requestField+"." + core.TEXTAREA_CR
...
func Project_Validate(r dm.Project) (dm.Project, error) {
	...
	r.OriginName,r.OriginName_props = Project_OriginName_validate_impl (PUT,r.ProjectID,r.OriginName,r,r.OriginName_props)
	...
	// Cross Validation
	var errVal error
	r, _, errVal = Project_ObjectValidation_impl(PUT, r.ProjectID, r)
	if errVal != nil {
		err = errVal
	}
	return r,err
