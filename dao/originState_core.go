...
func OriginState_Delete(id string) {
	OriginState_SoftDelete(id)
}

// OriginState_SoftDelete(id string) soft deletes a single OriginState record
...
// OriginState_HardDelete(id string) soft deletes a single OriginState record
func OriginState_HardDelete(id string) {
	// Uses Hard Delete
...
// OriginState_StoreProcess() saves/stores a OriginState record to the database
func OriginState_StoreProcess(r dm.OriginState, operator string) (dm.OriginState, error) {

	r, err := OriginState_Validate(r)
	if err == nil {
		err = originstate_Save(r, operator)
	} else {
		logs.Information("OriginState_StoreProcess()", err.Error())
	}
...
func originstate_Save(r dm.OriginState, usr string) error {
	...
	OriginState_HardDelete(r.OriginStateID)
	das.Execute(tsql)
