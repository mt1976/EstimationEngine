...
func Session_Delete(id string) {
	Session_HardDelete(id)
}

// Session_HardDelete(id string) soft deletes a single Session record
func Session_HardDelete(id string) {
	// Uses Hard Delete
	...
	das.Execute(tsql)
	//if err != nil {
	//	logs.Error("Session_SoftDelete()",err)
	//}
}
...
// Session_StoreProcess() saves/stores a Session record to the database
func Session_StoreProcess(r dm.Session, operator string) (dm.Session, error) {

	r, err := Session_Validate(r)
	if err == nil {
		err = session_Save(r, operator)
	} else {
		logs.Information("Session_StoreProcess()", err.Error())
	}
