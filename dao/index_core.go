...
func index_Save(r dm.Index, usr string) error {
	...
	// If there are fields below, create the methods in dao\index_impl.go  r.IndexID,err = Index_IndexID_OnStore_impl (r.IndexID,r,usr)  r.Link,err = Index_Link_OnStore_impl (r.Link,r,usr)
...
func index_Fetch(tsql string) (int, []dm.Index, dm.Index, error) {
	...
	for i := 0; i < noitems; i++ {
		...
		// If there are fields below, create the methods in adaptor\Index_impl.go   recItem.IndexID  = Index_IndexID_OnFetch_impl (recItem)   recItem.Link  = Index_Link_OnFetch_impl (recItem)//
		// Dynamically generated 15/02/2023 by matttownsend (Matt Townsend) on silicon.local
		// END
		///
		//Add to the list
