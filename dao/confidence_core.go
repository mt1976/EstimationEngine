...
func Confidence_GetFilteredLookup(requestObject string, requestField string) []dm.Lookup_Item {
	...
	usage := "Defines a filter for a lookup list of Confidence records, when requested by " + requestField + "." + core.TEXTAREA_CR
...
func Confidence_Validate(r dm.Confidence) (dm.Confidence, error) {
	...
	// Cross Validation
	var errVal error
	r, _, errVal = Confidence_ObjectValidation_impl(PUT, r.ConfidenceID, r)
	if errVal != nil {
		err = errVal
	}
	return r, err
