...
// ----------------------------------------------------------------
// Automatically generated  "/application/externalmessage_api.go"
// ----------------------------------------------------------------
// Package              : application
...
//ExternalMessage_Handler is the handler for the api calls
...
// Handles GET requests for ExternalMessage
func externalmessage_MethodGet(w http.ResponseWriter, r *http.Request) {
	...
	if searchID == "" {
		...
		noRecs, records, _ := dao.ExternalMessage_GetList()
		var ci core.ContentList
		ci.Count = noRecs
		ci.Key = dm.ExternalMessage_QueryString
		for _, v := range records {
			ciContent := core.ContentListItem{ID:v.MessageID,Query:"?" + ci.Key +"="+ v.MessageID}
...
//Handles POST & PUT requests for ExternalMessage
func externalmessage_MethodPost(w http.ResponseWriter, r *http.Request) {
	...
	_,err = dao.ExternalMessage_StoreSystem(t)
	if err != nil {
